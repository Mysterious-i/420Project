...
int main(int argc, char *argv[]) {
    ...
    for (int row = 0; row < height; ++row)  
        ...
        for (int col = 0; col < width; ++col) {
            ...
            c.imag = imag_min + ((double) (height-row) * scale_imag);
    ...
    char *filen = "seq.txt";
    record(filen,1,maxiter,total_t);
    
    
    /*take a screenshot*/
    
    const char *f = "screencapture ";
    char iter_s[10];
    sprintf(iter_s, "%d", maxiter);
    const char *s = ".png";
    size_t len_f = strlen(f);
    size_t len_i = strlen(iter_s);
    size_t len_s = strlen(s);
    size_t len_total = len_f + len_s + len_i;
    char *tmp = malloc(len_total + 1);
    strncpy(tmp, f, len_f);
    strncpy(tmp+len_f, iter_s, len_i);
    strncpy(tmp+len_f+len_i, s, len_s);
    tmp[len_total] = '\0';
    printf("\n %s \n", tmp);
    system(tmp);
    free(tmp);
