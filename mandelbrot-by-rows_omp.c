...
int main(int argc, char *argv[]) {
    ...
    if (argc > 2) {
        nthreads = get_integer_arg(argc, argv, 2, 1, "number_of_threads", usage_msg);
        double x0 = get_floating_arg(argc, argv, 3, -DBL_MAX, "x0", usage_msg);
        double y0 = get_floating_arg(argc, argv, 4, -DBL_MAX, "y0", usage_msg);
        double size = get_floating_arg(argc, argv, 5, 0, "size", usage_msg);
        real_min = x0 - size;
        real_max = x0 + size;
        imag_min = y0 - size;
        imag_max = y0 + size;
...
#pragma omp single
    ...
    fprintf(stdout, "\n");
    char *filen = "omp-r.txt";
    record(filen,nthreads,maxiter,total_t);
        sleep(2);
